...
import { getTOTP, getCredentials } from "../helpers/Utils.js";

Given("the {} user login to Elsie app", async (role) => {
  const { username, password, secret } = getCredentials(role);
  ...
  await IjamLoginScreen.submitLoginForm({ password: password });

  const totp = getTOTP("Microsoft", username, "SHA1", 6, 30, secret);
  if (driver.isAndroid) {
    await driver.switchContext("WEBVIEW_com.betalabs.elsie.staging");
  }
  await OTCScreen.waitForIsShown();
...
When("the user login with Microsoft account", async (credential: DataTable) => {
  ...
  await OnboardSrceen.tapOnSignInButton();
  await MSLoginScreen.waitForIsShown();
  const username = credential.hashes()[0].username
    ? credential.hashes()[0].username
    : process.env.MANAGER_USERNAME;
  const password = credential.hashes()[0].password
    ? credential.hashes()[0].password
    : process.env.MANAGER_PASSWORD;
  await MSLoginScreen.submitMSEmail({ username: username });
  await IjamLoginScreen.waitForIsShown();
  await IjamLoginScreen.submitLoginForm({
    username: username,
    password: password,
  });
});
...
Then("the user should see a login failure message", async () => {
  ...
  expect(await IjamLoginScreen.waitForIsShown).toBeTruthy();
});
